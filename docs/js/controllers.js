...
aggregationControllers.controller('AggregationListCtrl', ['$scope', '$http', 'client', 'esFactory', 'Init', '$routeParams', '$route', '$location',
  ...
  function ($scope, $http, client, esFactory, Init, $routeParams, $route, $location) {
    ...
    $scope.userFilters = Init.userFilters();
    ...
    $scope.removeFacet = function(facet) {
      ...
      for ($scope.i = 0; $scope.i < $scope.selectedFacets.length; $scope.i++) {
        if (facet.id != $scope.selectedFacets[$scope.i].id) {
      ...
      console.log('Removed facet ');
      console.log(facet);

      console.log(Init.selectedFacets());
    ...
    */
    $scope.expandCollapseFilter = function(f) {
     for (var i=0; i<userFilters.length;i++) {
      if (f==userFilters[i]) {
        if (userFilters[i].class=='hidden') {
          userFilters[i].class='on';
        } else {
          userFilters[i].class='hidden';
        }
        return;
      }
     }
    ...
    $scope.toggleFilter = function(tag,filter) {
      tag.checked = ! tag.checked;

      if (filter.type=='facet') {
        var facet = [];

        facet['title'] = tag.value;
        facet['id'] = tag.id;
        facet['field'] = filter.facet;
        facet['count'] = 212;

        if (tag.checked) {
          $scope.addFacet(facet);
    ...
    $scope.getSuperFormat = function(longFormat, id) {
        if (typeof(id)=='undefined') {
          ...
          if (typeof($scope.fileInfo[id])!='undefined' && typeof($scope.fileInfo[id].original)!='undefined') {
            return ($scope.getFormat($scope.fileInfo[id].original.format));
          }
          return 'unknown';
        }
        return 'unknown-3';
    ...
    $scope.updateResults = function() {
      ...
      console.log('Malakas nalakakaka  ');

      if ($scope.selectedFacets.length > 0) {
        console.log('Got filters in a bag ');
      ...
      request.success(
        function(data) {
          ...
          if (data.bitstreams[0].bundleName == 'URL_BUNDLE') {
            ...
            $scope.url = 'http://'+    location.hostname+'/api/preview/'+path;
          }
